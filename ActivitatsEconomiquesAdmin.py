...
#       *****************************************************************************************************************
        if (self.dlg.RelacionarPoblacio.isChecked()):
            if Auto_generacio==False or (Auto_generacio==True and Auto_primera_vegada==True):
                path_file = QFileDialog.getExistingDirectory(self.dlg,
            ...
            while trobat:
                if (path_file != ''):
                    if (os.path.exists(path_file + "\\tr_illes.csv")):
                        ...
                        path_file = QFileDialog.getExistingDirectory(self.dlg,"Busca la carpeta que conté els arxius provinents del mòdul TAULA RESUM", Path_Inicial+"\\",QFileDialog.ShowDirsOnly)
                else:
                    print ("Cancelat")
                    trobat = False
                    self.dlg.Progres.setVisible(False)
        ...
        if (self.dlg.EstatConnexio.text()=='Connectat'): 
            ...
            if (len(llista_sel)>0 or len(llista_sel_EPIGRAF)>0):
                ...
                if Auto_generacio==False:
                    ...
                    cur.execute("DROP TABLE IF EXISTS \""+titol3.decode('utf8')+"\";")
                    conn.commit()
                    cur.execute("CREATE TABLE \""+titol3.decode('utf8')+"\" AS ("+sql_total+");")
                    conn.commit()
                    cur.execute("COMMENT ON TABLE \""+titol3.decode('utf8')+"\" IS '"+titol3.decode('utf8')+" "+mes[date.today().month]+" "+str(date.today().year)+"';")
                    conn.commit()
                #print uri.uri()
...
#                       *****************************************************************************************************************
                ...
                if (self.dlg.RelacionarPoblacio.isChecked()):
                    ...
                    if Auto_generacio==False:
                        ...
                        titol3=titol2+titol
                        vlayer = QgsVectorLayer(uri.uri(), titol3, "postgres")
                    else:
                        if (self.dlg.GrafCombo.currentText()=="Distancia" or self.dlg.ZICirc_radio.isChecked()):
                            titol2=self.dlg.Radi_ZI.text().encode('utf8','strict')
                            #titol3=titol.encode('utf8','strict')+"_"+titol2.encode('utf8','strict')+u'm_PE'
                            titol3=titol+"_"+titol2.decode('utf8')+u'm_PE'
                        else:
                            titol3=titol+"_"+self.dlg.Radi_ZI.text()+u'min_PE'                            
                    ...
                    if vlayer.isValid():
                        ...
                        vlayer = QgsVectorLayer(os.environ['TMP']+"/Tematic_"+Tematic+".shp", titol3, "ogr")
                        #self.eliminaTaulesCalcul(Fitxer)
                        fieldname="Habitants"
                        numberOfClasses=5
                        myRangeList=[]
                ...
                if (self.dlg.Veure_ZI.isChecked()):
                    if not(self.dlg.NoMostrarZI.isChecked()):
                        ...
                        if Auto_generacio==False:
                            ...
                            if (self.dlg.GrafCombo.currentText()=="Distancia" or self.dlg.ZICirc_radio.isChecked()):
                                titol2=self.dlg.Radi_ZI.text().encode('utf8','strict')
                                titol3=titol+"_"+titol2.decode('utf8')+u'm_ZI'
                            else:
                                titol3=titol+"_"+self.dlg.Radi_ZI.text()+u'min_ZI'                            
                            cur.execute("DROP TABLE IF EXISTS \""+titol3+"\";")
                            conn.commit()
